    ...
    public class Day08
    {
        ...
        public static void Part1()
        {
            ...
            foreach (var line in lines)
            {
                ...
                int slashQuoteCnt = 0;
                int slashXCnt = 0;
                int quoteCnt = 2; //first and last quotes.
                totalCode += line.Length;
                for (int i = 0; i < line.Length - 1; i++)
                {
                    string twoChar = line[i].ToString() + line[i + 1].ToString();
                    if (twoChar.Contains(@"\\"))
                    {
                        dblSlashCnt++;
                        i += 2;
                    ...
                    string slashQuote = """
                        \"
                        """;
                    if (twoChar.Contains(slashQuote))
                    {
                        slashQuoteCnt++;
                        i += 2;
                    }

                    if (twoChar.Contains(@"\x"))
                    {
                        slashXCnt += 3;
                        i += 4;
                    }

                    //dblSlashCnt += twoChar.Contains(@"\\") ? 1 : 0;
                    //slashQuoteCnt += twoChar.Contains(@"\" + "\"") ? 1 : 0;
                    //slashXCnt += twoChar.Contains(@"\x") ? 3 : 0;
                }

                int charCnt = dblSlashCnt + slashQuoteCnt + slashXCnt + quoteCnt;
                totalChar += charCnt;

                Console.WriteLine(line);
                Console.WriteLine($"dblSlashCnt: {dblSlashCnt}");
                Console.WriteLine($"slashQuoteCnt: {slashQuoteCnt}");
                Console.WriteLine($"slashXCnt: {slashXCnt}");
                Console.WriteLine($"{line.Length} - {charCnt} = {line.Length - charCnt}");
                
            }
            Console.WriteLine($"Totals: {totalCode} - {totalChar} = {totalCode - totalChar}");
